<template>
  ...
  >
    ...
    <div class="i-answer-detail-tit">
      {{ propData.title }}
    </div>
    <div
      class="i-answer-detail-content-item answer"
      v-for="(answer, index) in answerList"
      :key="index"
    >
      <div class="answer-wrapper">
        <div class="i-answer-detail-content-left">
          <img :src="getImageSrc(answer)" class="content-left-avatar" />
        </div>
        <div class="i-answer-detail-content-right">
          <div class="content-right-top answer">
            <span class="comment-name">{{
              answer[propData.avatarInterface]
            }}</span>
            <span class="comment-time"
              >发布于 {{ answer[propData.timeInterface] }}</span
            >
          </div>
          <p class="content-right-center">
            {{ answer[propData.btInterface] }}
          </p>
          <div class="content-right-bottom">
            <div class="comment-date-from">
              <span
                @click="
                  likeClick(
                    answer[propData.starInterface] ? 'unlike' : 'like',
                    answer
                  )
                "
                :class="{ active: answer[propData.starInterface] }"
                >{{ answer[propData.starNumInterface] }} 赞同</span
              >
            </div>
            <div class="comment-delete-star answer">
              <span @click="toggleComment(answer)">{{
                answer.commentActive ? "收起评论" : "展开评论"
              }}</span>
              <!-- v-if="userInfo.userid && item.fromUserId == userInfo.userid" -->
              <span @click="delClick(answer, i)"> 删除 </span>
              <span>{{answer.isAccept ? '已采纳' : '采纳'}}</span>
            </div>
          </div>
        </div>
      </div>
      <div class="anwer-comment" v-if="answer.commentActive">
        <div class="i-answer-detail-textarea">
          <van-field
            v-model="answer.curComment"
            rows="3"
            autosize
            type="textarea"
            placeholder="请输入评论"
          />
          <van-button type="info" @click="publishClick(answer)">发布</van-button>
        </div>
        <div class="i-answer-detail-header">
          <div class="i-answer-detail-header-left">
            <span v-if="propData.showTotal" class="header-left-number"
              >{{ answer[propData.totalInterface] }}条评论</span
            >
          </div>
        </div>

        <div class="i-answer-detail-content">
          <div
            class="i-answer-detail-content-item"
            v-for="(item, i) in answer.commentList"
            :key="item.id"
          >
            <div class="i-answer-detail-content-left">
            ...
            <div class="i-answer-detail-content-right">
              <div class="content-right-top">
                ...
                <span class="comment-time" v-if="propData.showCommentTime">{{
              ...
              <div class="content-right-bottom">
                ...
                <div class="comment-delete-star">
                  ...
                  >
                  <span v-if="propData.showCommentBtn" @click="replyClick(item)"
                    ><svg-icon icon-class="comment"
                  /></span>
                  <span v-if="propData.showTrampleBtn"
                    ><svg-icon icon-class="trample" />{{
                      item[propData.trampleNumInterface]
                    }}</span
                  >
                  <span
                    v-if="userInfo.userid && item.fromUserId == userInfo.userid"
          ...
          <a-empty v-if="answer[propData.totalInterface] == 0" :description="'暂无数据'" :image="simpleImage" />
          <div v-if="total > 3" class="i-answer-detail-content-more">
            查看全部评论
          </div>
        </div>
      </div>
...
<script>
import { List, Icon, Field, Button } from "vant";
...
const mock = [
  {
    ...
    answerUserName: "成成",
    answerAvatar: "",
    answer: "准备身份证、学士学位、报道证等材料",
    answerTime: "2022-08-07",
    isAccept: true,
    commentList: [
      {
        avatar: "",
...
export default {
  ...
  data() {
    return {
      ...
      propData: this.$root.propData.compositeAttr || {
        ...
        finishedTip: "数据已加载全部",
        imgInterface: "answerAvatar",
        idInterface: "id",
        avatarInterface: "answerUserName",
        timeInterface: "answerTime",
        starInterface: "like",
        starNumInterface: "likeNum",
        trampleNumInterface: "trampleNum",
        btInterface: "answer",
        dateInterface: "date",
        totalInterface: "commentNum",
        showDelBtn: true,
        showStarBtnBottom: true,
        showCommentBtn: true,
        showTrampleBtn: false,
        delBtnFiled: "showDel",
        keepAlive: true,
        showTotal: true,
  ...
  methods: {
    ...
    convertAttrToStyleObject(pageSize = {}) {
      ...
      styleObject["--i-answer-detail-scale"] = scale;
      ...
      window.IDM.setStyleToPageHead(
        this.moduleObject.id +
          ` .i-answer-detail-header .i-answer-detail-header-left`,
        titleStyleObject
      );
      window.IDM.setStyleToPageHead(
        this.moduleObject.id + ` .i-answer-detail-content`,
        innerStyleObject
      );
      this.initData()
    },
      ...
      for (var i = 0; i < themeList.length; i++) {
        ...
        IDM.setStyleToPageHead(
          "." +
            ...
            " .i-answer-detail-header .i-answer-detail-header-right",
          {
        ...
        IDM.setStyleToPageHead(
          "." +
            ...
            " #" +
            (this.moduleObject.packageid || "module_demo") +
            " .i-answer-detail-header .header-right-btn",
          {
        ...
        IDM.setStyleToPageHead(
          "." +
            ...
            " #" +
            (this.moduleObject.packageid || "module_demo") +
            " .i-answer-detail-header .header-right-btn.active",
          {
        ...
        IDM.setStyleToPageHead(
          "." +
            ...
            " #" +
            (this.moduleObject.packageid || "module_demo") +
            " .i-answer-detail-outer .i-answer-detail-textarea .van-button",
          {
            "background-color": item.mainColor
              ? IDM.hex8ToRgbaString(item.mainColor.hex8)
              : "",
        ...
        IDM.datasource.request(
          dataSource.id,
          {
            ...
            param: {
              type: "acceptList",
              params: {
                contentId: IDM.url.queryString("questionId"),
                ...this.setCustomParams(),
              },
            },
          },
          (res) => {
            console.log(res, "回答列表接口返回结果");
            this.dealList(res);
          },
          (res) => {
    ...
    receiveBroadcastMessage(messageObject) {
      console.log("全部评论组件收到消息", messageObject);
...
$scale: var(--i-answer-detail-scale);

.i-answer-detail-outer {
  position: relative;
  padding: calc(10px * #{$scale});
  color: #000;
  font-size: calc(14px * #{$scale});
  background-color: #fff;
  margin: 0 calc(10px * #{$scale});
  ...
  .i-answer-detail-header {
    ...
    .i-answer-detail-header-left {
      font-size: calc(16px * #{$scale});
    }
  }

  .i-answer-detail-textarea {
    display: flex;
    ...
    .van-field {
      ...
      padding: calc(6px * #{$scale}) calc(8px * #{$scale});
    }
  ...
  .i-answer-detail-content {
    .i-answer-detail-content-more {
      color: #0954c6;
      text-align: center;
      padding: calc(10px * #{$scale}) 0;
    }
  ...
  .i-answer-detail-content-item {
    display: flex;
    border-bottom: 1px solid rgb(222, 222, 222);
    margin-left: calc(20px * #{$scale});
    padding:calc(6px * #{$scale}) 0;

    &.answer {
      display: block;
      margin-bottom: calc(10px * #{$scale});
      margin-left: 0;
